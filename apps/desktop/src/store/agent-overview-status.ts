...
const attentionFor = (status: SessionDotState): Attention => {
  ...
  // rate-limited is this fork's own bucket (Phase 2.12): a turn stalled on a
  // manual recovery choice reads as needs-you here, same as needs-input.
  return bucket === 'needs-input' || bucket === 'rate-limited' ? 'needs-you' : bucket === 'draft' ? 'idle' : bucket
}
