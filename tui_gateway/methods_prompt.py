...
def _(rid, params: dict) -> dict:
    ...
    Whole-file transport, kept for callers that already hold the complete
    ``data_url`` in memory (small files, and older desktop builds that predate
    the chunked file.attach_open/_chunk/_commit path below). New large-file
    uploads should prefer the chunked RPCs so no single JSON-RPC frame ever
    carries a whole-file base64 payload.
    """
    session, err = _sess_building(params, rid)
    if err:
        return err
