...
pending row for the exact task/run/pid identity immediately before
`enforce_max_runtime` calls `kill()` (updating it if the SIGKILL grace period
is hit). `_classify_dead_worker` checks that exact identity before consulting
