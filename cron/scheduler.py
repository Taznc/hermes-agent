...
def _prepare_job_prompt(
    ...
    # resume_session_id short-circuit — this job re-submits its stored prompt into an EXISTING
    # local session instead of spawning a fresh cron_{job_id}_{timestamp} AIAgent. Checked
    # before the empty-payload guard (a resume job's whole point is a short "retry"
    # instruction, never a script/skill payload) and before any AIAgent/SessionDB construction.
    if job.get("resume_session_id"):
        return _run_resume_session_job(job, job_id, job_name, extra_prompt=extra_prompt), None
