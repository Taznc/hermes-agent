...
def test_desktop_launch_options_normalizes_auto_update_checks_enabled(raw, expected):
    cfg = {"desktop": {"auto_update_checks_enabled": raw}}
    with patch("hermes_cli.config.load_config", return_value=cfg):
        assert main_desktop._desktop_launch_options()[4] is expected


def test_desktop_launch_options_auto_update_checks_defaults_enabled():
    with patch("hermes_cli.config.load_config", return_value={}):
        assert main_desktop._desktop_launch_options()[4] is True


def test_gui_bridges_disabled_update_checks_to_launch_env(tmp_path, monkeypatch):
    """``desktop.auto_update_checks_enabled: false`` sets
    ``HERMES_DESKTOP_DISABLE_UPDATE_CHECKS=1`` on the launched Electron process,
    and an explicit env var always wins over config."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    monkeypatch.delenv("HERMES_DESKTOP_DISABLE_UPDATE_CHECKS", raising=False)

    ok = subprocess.CompletedProcess([], 0)
    cfg = {"desktop": {"auto_update_checks_enabled": False}}
    ...
    assert launch_env.get("HERMES_DESKTOP_DISABLE_UPDATE_CHECKS") == "1"

    # Default config (auto_update_checks_enabled=True) never sets the env var.
    with patch("hermes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("hermes_cli.main_web_build._run_npm_install_deterministic", return_value=ok), \
         patch("hermes_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("hermes_cli.main_desktop._write_desktop_build_stamp"), \
    ...
    assert "HERMES_DESKTOP_DISABLE_UPDATE_CHECKS" not in launch_env
