...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `hermes <plugin> <subcommand>`; add `parent="kanban"` to nest the action under a built-in command instead |
