...
| `kanban.count_infra_failures` | `false` | When `false` (default), a worker death classified as **infra** — an external SIGKILL/SIGTERM the dispatcher did not send itself, a dead pid discovered inside the dispatcher's own startup window (gateway restart / VM boot), or a provider 429/quota signature — does NOT increment `consecutive_failures` and is recorded as an `interrupted` event instead of `crashed`/`gave_up`. Set `true` to restore pre-classification behaviour (every such death counts like any other crash). The dispatcher's own `--max-runtime` kill is never affected by this setting — it always counts. |
| `kanban.infra_startup_window_seconds` | `120` | How long after the dispatcher process itself started a "pid N not alive" discovery is presumed to be a gateway restart / VM boot (classified `infra`) rather than a genuine crash (classified `legit`). Only consulted when `count_infra_failures` is `false`. Also overridable via `HERMES_KANBAN_INFRA_STARTUP_WINDOW_SECONDS`. |
...
| `interrupted` | `{pid, claimer, reason, quota_retry_after_seconds?}` | Worker PID no longer alive, classified `infra` instead of a genuine crash — an external SIGKILL/SIGTERM the dispatcher did not send itself (`reason: "external_signal"`), a dead pid discovered inside the dispatcher's own startup window (`reason: "startup_window"`, likely a gateway restart / VM boot), or a provider 429/quota signature (`reason: "quota"`, with `quota_retry_after_seconds` when parseable from the log). Does NOT increment `consecutive_failures`; the task returns to `ready` immediately. Gated by `kanban.count_infra_failures` (default `false` — set `true` to fold these back into `crashed`/`gave_up` like before this classification existed). See [Infra-death classification](https://github.com/Taznc/hermes-agent/blob/dev/docs/kanban/infra-failure-classification.md). |
