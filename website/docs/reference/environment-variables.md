...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly (highest precedence; beats `HERMES_KANBAN_BOARD` and `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board. Honored only when it resolves under the currently-active kanban home (`HERMES_KANBAN_HOME` / `get_default_hermes_root()`); a pin computed against a *different* home (e.g. inherited by a script that repoints `HERMES_HOME` to a sandbox without also clearing this var) is dropped with a warning instead of silently redirecting writes to the wrong home |
| `HERMES_KANBAN_WORKSPACES_ROOT` | Pin the kanban workspaces root directly (highest precedence for workspaces; beats `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env. Same stale-pin protection as `HERMES_KANBAN_DB` |
| `HERMES_KANBAN_ATTACHMENTS_ROOT` | Pin the kanban attachments root directly (highest precedence for attachments; beats `HERMES_KANBAN_HOME`). Same stale-pin protection as `HERMES_KANBAN_DB` |
