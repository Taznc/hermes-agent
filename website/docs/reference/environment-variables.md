...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly (beats `HERMES_KANBAN_BOARD` and `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board, alongside `HERMES_KANBAN_PIN_HOME` |
| `HERMES_KANBAN_PIN_HOME` | Provenance stamp written beside a dispatcher-injected `HERMES_KANBAN_DB`: the kanban home the pin was computed under. When a process re-declares a *different* home (`HERMES_HOME` / `HERMES_KANBAN_HOME`), the stamped pin is treated as stale inheritance and ignored, with a warning — this is what lets a probe or test script sandbox itself instead of silently opening the live board. A hand-set pin carries no stamp and always wins |
