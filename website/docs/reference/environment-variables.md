...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly (beats `HERMES_KANBAN_BOARD` and `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board. Honored only when it resolves under the currently-active kanban home (`HERMES_KANBAN_HOME` / `get_default_hermes_root()`), or when `HERMES_KANBAN_PIN_HOME` vouches for it; a pin computed against a *different* home (e.g. inherited by a script that repoints `HERMES_HOME` to a sandbox without also clearing this var) is dropped with a warning instead of silently redirecting writes to the wrong home |
| `HERMES_KANBAN_PIN_HOME` | Names the kanban home a `HERMES_KANBAN_*` path pin is valid under, allowing a pin that deliberately points *outside* the current home (symlink / Docker layouts where the board does not live under the resolved home). The dispatcher writes it beside the pins it injects. When a process re-declares a different home (`HERMES_HOME` / `HERMES_KANBAN_HOME`) the stamp no longer matches, so inherited pins are dropped — this is what lets a probe or test script sandbox itself instead of silently opening the live board |
| `HERMES_KANBAN_WORKSPACES_ROOT` | Pin the kanban workspaces root directly (beats `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env. Same stale-pin protection as `HERMES_KANBAN_DB` |
| `HERMES_KANBAN_ATTACHMENTS_ROOT` | Pin the kanban attachments root directly (beats `HERMES_KANBAN_HOME`). Same stale-pin protection as `HERMES_KANBAN_DB` |
