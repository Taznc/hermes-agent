...
1. Read `~/projects/hermes/docs/feature-ledger.md`. If the feature is listed, follow its Decision (PORT / WAIT / DROP) and target tier.
2. Search upstream: `GH_CONFIG_DIR=/home/hermes/.config/gh gh search prs --repo NousResearch/hermes-agent "<feature>"` and `git log upstream/main --grep`. Upstream may have shipped it.
3. Check `dev` for prior art: `git log origin/dev --oneline -- <path>` and `git show origin/dev:<path>`. Ports **re-implement at the lowest tier**; cherry-pick only when the source commit is entirely in T1 dirs.
4. Query codebase-memory project `home-hermes-projects-hermes-hermes-agent-next` for architecture/callers before broad reading. `home-hermes-projects-hermes-hermes-agent-dev` is the read-only index of `dev` for port lookups. Never `index_repository` from a worktree.
