...
def predict_auto_provider(profile: str) -> Optional[str]:
    ...
    Mirrors the worker's own startup ladder — the explicit ``provider=auto``
    request passed by the dispatcher and then :func:`hermes_cli.auth.resolve_provider`
    — under the profile's Hermes home, so the answer reflects that profile's
    ``.env`` and OAuth state rather than the dispatcher's. Returns ``None`` when
    resolution fails or is unavailable; callers treat that as unprovable and
    fail closed.
    """
    ...
    try:
        ...
        provider = resolve_requested_provider("auto")
        if provider == "auto":
