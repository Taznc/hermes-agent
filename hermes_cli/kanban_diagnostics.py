...
# Human-facing copy per guard reason: (title, detail-template, expiry-window-
# seconds-or-None). ``{age_str}`` and ``{expiry}`` are filled in by the rule.
_RESPAWN_GUARD_COPY = {
    "recent_success": (
...
def _latest_respawn_guard_event(events: Iterable[Any]) -> Optional[Any]:
    """Most recent ``respawn_guarded`` event, or ``None``."""
    latest = None
    latest_ts = -1
    for ev in events:
...
def _rule_respawn_guarded(task, events, runs, now, cfg) -> list[Diagnostic]:
    """Surface a RECENT, benign ``respawn_guarded`` event (``recent_success``,
    ``active_pr``, ``rate_limit_cooldown``) as a low-severity informational
    diagnostic instead of leaving it write-only. This is what keeps a
    guard-held ``ready`` task from reading as unexplained: ``_rule_stranded_in_ready``
    suppresses itself for these same reasons, so this rule is the ONLY signal
    the operator sees for a guard-held task.

    Deliberately informational, never warning/error/critical: the guard is
    the dispatcher working as designed, not a fault. ``blocker_auth`` is NOT
    covered here — it is a real, operator-fixable problem and stays inside
    ``_rule_stranded_in_ready`` (with the correct cause and actions) so it is
    never lost, downgraded, or duplicated.
    ...
    if _task_field(task, "status") != "ready":
        return []
    guard_ev = _latest_respawn_guard_event(events)
    if guard_ev is None:
        return []
    ...
    if reason not in _RESPAWN_GUARD_BENIGN_REASONS:
        return []
    ...
    actions: list[DiagnosticAction] = []
    if reason == "active_pr":
        # No reassign/unblock/reclaim here — any of those would defeat the
        # very guard this diagnostic is reporting on (duplicate-PR risk).
        actions.append(_cli_hint("Check dispatcher status", "hermes kanban diagnostics"))
        if task_id:
            actions.append(_cli_hint(f"Review comments for the PR link: hermes kanban show {task_id}",
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    Before blaming a missing/misnamed worker, this consults the latest
