...
DEFAULT_CONFIG = {
    ...
    "updates": {
        # Whether Hermes may check upstream (GitHub) for newer commits/releases at all — the startup
        # banner prefetch, `hermes --version`, the TUI gateway prefetch, and the dashboard's cached
        # System-page check all gate on this one flag. false fully silences upstream contact for
    ...
    "computer_use": {
        ...
        "cua_telemetry": False,
        # Whether the backend may background-check trycua/cua's GitHub Releases for a newer
        # cua-driver build (`_maybe_nudge_update()`, once per process, ~20h cache). Distinct from
        # Hermes' own `updates.check_for_updates` — this asks about the third-party driver binary.
        # `HERMES_DEV=1` disables it too. Explicit `hermes computer-use status`/`install --upgrade`
        # checks are user-initiated and always run.
    ...
    "desktop": {  # Hermes Desktop (Electron) launch options; only affect `hermes desktop`.
        ...
        "electron_flags": [],
        # Whether the Desktop app may contact upstream (git ls-remote/fetch + GitHub compare API)
        # to check for client self-updates: the background poller (startup + 30-minute interval +
        # window-focus recheck) and the "Check for Updates..." menu item. false stops a dev checkout
        # pinging upstream — bridged to HERMES_DESKTOP_DISABLE_UPDATE_CHECKS (explicit env wins).
        # Does not affect `hermes update` run manually from the CLI.
...
# >>> FORK ANCHOR: kanban-config-defaults <<<
# Fork additions to DEFAULT_CONFIG["kanban"] (infra-failure classification, provider-quota
# backoff, review routing, priority reservation, post-drain actions, review-round capping,
# worker-launcher decoupling) live in hermes_fork/kanban/config_defaults.py and are merged
# here — literal new keys on upstream's dict, not extractable logic. DEFAULT_CONFIG is
# deep-copied/looked up by key everywhere, never by literal order, so this is safe.
from hermes_fork.kanban import config_defaults as _fork_kanban_config_defaults  # noqa: E402

DEFAULT_CONFIG["kanban"].update(_fork_kanban_config_defaults.FORK_KANBAN_DEFAULTS)
...
# >>> FORK ANCHOR: kanban-config-defaults <<<
# HERMES_KANBAN_* env bridges for the kanban.* keys merged above; built via a factory
# call, not a direct import, because `_setting` is defined only in this module.
OPTIONAL_ENV_VARS.update(_fork_kanban_config_defaults.kanban_env_var_overrides(_setting))
