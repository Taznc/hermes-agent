...
KANBAN_GUIDANCE = (
    ...
    "`kanban_request_review(summary=..., metadata=..., reviewer=<optional-profile>)`. Name a `reviewer` other than "
    "the current assignee — a review dispatched back to the profile that just implemented the card finds nothing "
    "left to do, exits cleanly, and gets scored a protocol violation. Omitting `reviewer` on a board with "
    "`kanban.default_reviewer` configured lets the dispatcher route the card to that profile automatically; leave "
    "it unset unless you have a specific reviewer in mind. Reviewers approve with `kanban_complete` or return all "
    "correctable defects via `kanban_request_changes` with `reason=..., blockers=[{basis, reference}, ...]`. Use the four "
    "schema bases. Re-review accepts cited blockers and `base_regression` items linked by `rework_of`; followups stay "
    "inert. Ready review children create/link separate repair ahead of review, then request changes; never repair in-card. "
    "Block only on external prerequisites.\n"
    "6. **Route required followups.** Optional suggestions are inert. Route required out-of-scope work to a "
    "separate assignee; don't scope-creep.\n"
    ...
    "\n"
    "**Declare each card's edit surface.** When two cards you fan out would touch the same file, prose in both bodies "
    "does NOT prevent the collision — the workers run in separate worktrees and cannot see each other. Give every "
    "child card body an `Edit-Targets: <path>, <path>` line naming the files it will write. The dispatcher reads it "
    "and auto-serializes any two cards that name a common path (the second gains a dependency edge on the first and "
    "starts from a tree containing its work), so declaring costs nothing when there is no overlap and prevents an "
    "add/add conflict when there is.\n"
    "\n"
